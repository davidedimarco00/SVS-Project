...
class RadarManager:
    def __init__(self, vehicle, world):
        """
        :param vehicle: l'ego vehicle a cui attaccare i radar
        :param world:  riferimento al mondo di CARLA
        """
        ...
        self.avoiding_collision = False  # Flag per evitare continui cambi di direzione


    def spawn_radars(self):
        # Impostazioni base del radar: FOV e range
        ...
        # Spawna i radar e associa i callback
    ...
    def radar_callback_left(self, radar_data):
        """
        Lettura dati dal radar sinistro.
        Usa le coordinate locali (x,y,z) del radar per
        definire un'area di Blind Spot dietro/sinistra.
        """
        ...
        for detection in radar_data:
            ...
            # Esempio: definisco un box dove x: da 0 a 10 (10m dietro)
            #                            y: da -3 a -0.5 (lato sinistro)
    ...
    def radar_callback_lateral(self, radar_data):
        ...
        for detection in radar_data:
            ...
            # Coordinate locali al sensore:
            # x -> direzione in cui "punta" il radar (destra)
            # y -> avanti/indietro rispetto al veicolo
            # z -> in alto/basso
            x = distance * math.cos(altitude) * math.cos(azimuth)
            y = distance * math.cos(altitude) * math.sin(azimuth)
            z = distance * math.sin(altitude)
            ...
            # ------------------------------------------------
            # Definizione dell'area LATERALE DESTRA
            # ------------------------------------------------
            # Esempio:
            #  - 0 < x < 2.5  -> oggetto entro 2.5 m di lato
